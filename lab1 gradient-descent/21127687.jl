...
md"""
## 1. Loss landscape
![Loss lanscape](https://lnhutnam.github.io/resources/images/GD.png) 
...
$$h_i = \mathbf{x}_i^T W \Rightarrow \frac{\partial h_i}{\partial W} = \mathbf{x}_i^T$$
...
$$\nabla Loss = \frac{\partial Loss(W)}{\partial W} =\frac{1}{n} \sum_{i=1}^n \frac{\partial loss(W)}{\partial \hat{y}_i}\frac{\partial \hat{y}_i}{\partial h_i} \frac{\partial h_i}{\partial W}$$

$$= \frac{2}{n} \sum_{i=1}^n(\hat{y}_i - y_i)(\hat{y}_i - \hat{y}_i^2)x_i^T $$

$$= \frac{2}{n} \sum_{i=1}^n\hat{y}_i^2x_i^T - \hat{y}_i^3x_i^T - \hat{y}_iy_ix_i^T + \hat{y}_i^2y_ix_i^T$$
"""
...
# ╔═╡ 2105cadb-481a-4e96-a7e6-6b666855b37d
Random.seed!(2024)
...
begin
	...
	X_train, y_train, X_test, y_test = train_data[:,1:3], train_data[:,4], test_data[:,1:3], test_data[:,4]
	# DOT NOT MODIFY THIS CODE
end

# ╔═╡ e1cb26f5-5f31-4f04-ba9f-8f6eb5cb1b32
begin
	plt = scatter(X_test'[1,:][y_test .== 0], X_test'[2,:][y_test .== 0], label="Class 0", color=:blue, legend=:topright, markersize=4) # assign to plt var
	scatter!(X_test'[1,:][y_test .== 1], X_test'[2,:][y_test .== 1], label="Class 1", color=:red, markersize=4)
end
...
begin
	...
	function sigmoid_deriv(x)
	    ...
	    """
		temp = x' * (1. .- x)
		#print("X dimension: ", string(size(x)))
		#print("Dimension: ", string(size(temp)))
	    return temp
	end

	#print(sigmoid_activation([3, 4, 5]))
	#print(sigmoid_deriv(sigmoid_activation([3, 4, 5])))
end
...
begin
	function compute_gradient(error, y_hat, trainX)
	    ...
	    """
		N = size(trainX, 1)
		deriv_error = sigmoid_deriv(y_hat)
		# features: trainX (n x d)
	    return trainX' * (error * deriv_error)* 2/N
	end
end
...
begin
	function train(W, trainX, trainY, learning_rate, num_epochs)
	    ...
	    for epoch in 1:num_epochs
			...
			println(compute_gradient(error, y_hat, trainX))
...
begin
	...
	print(size(W))
	#print(size(compute_h(W, X_train)),size(X_train), size(W))
	num_epochs=100
	#learning_rate=0.08
	#learning_rate = 0.1
	learning_rate = 0.01
...
begin
	#training model
	#print(sigmoid_activation(compute_h(W, X_train)))
	#print(sigmoid_activation(compute_h(W, X_train)) - y_train)
...
begin
	...
	# Display
	print("acc: $accuracy, precision: $precision, recall: $recall, f1_score: $f1_score\n")
end

# ╔═╡ 806ca12f-aee1-4765-b80d-37f6e7644b88
begin
	# Create a scatter plot
	scatter(X_test'[1,:][y_test .== 0], X_test'[2,:][y_test .== 0], label="Class 0", color=:blue, legend=:topright, markersize=4)
	scatter!(X_test'[1,:][y_test .== 1], X_test'[2,:][y_test .== 1], label="Class 1", color=:red, markersize=4)
	...
	D_test = ([
	  tuple.(eachcol(hcat(X_test'[1,:][y_test .== 0], X_test'[2,:][y_test .== 0])'), 1)
	  tuple.(eachcol(hcat(X_test'[1,:][y_test .== 1], X_test'[2,:][y_test .== 1])'), -1)
	])
...
- Recall: With the above assumption, recall represents the percentage of True Positive (TP) among those that belong to the Positive group (True Positive + False Negative). Hence the formula: recall = TP / TP + FN
- Precision: Unlike recall, precision represents the percentage of TP among those that are predicted as Positive, that includes the wrongly predicted Negative datapoints. We have the formula: precision = TP / TP + FP
...
The data is very sensitive to the learning rate and the initial weights generated. With the learning rate $\alpha = 0.1$, often times the model could not generate a sufficient prediction, reaching 60% to 80% in accuracy but generally leaning towards either the negative or positive samples. There are some cases the model was able to accurately give a decision bounddary that separates two groups from one another, but the succession rate is quite low. This is the same for many other learning rates such as 0.06, 0.05, ... 
The submitted version contains the visualization of a good run with learning rate $\alpha = 0.01$, as the model converges almost immediately. However, trying to use the same learning rate for another data generation might not produce the same results. 
