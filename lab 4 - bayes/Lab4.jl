...
# ╔═╡ e6e42d3a-b934-4d0f-8939-76cd92361bc7
current_dir = pwd()

# ╔═╡ 46a8dba1-dda1-4ea5-9937-172ad6439df9
...
begin
	...
	function entropy(counts, n_samples)
	    #TODO
		p_i = counts ./ n_samples
		return -1.0 .* sum(p_i .* log.(2, p_i))
	end
	
	print(entropy([5, 6], 11))
end
...
function entropy_of_one_division(division)
	...
	# count the occurrences of each unique element i
    counts=[count(==(i), division) for i in n_classes]
    #TODO:
	calc_entropy = entropy(counts, n_samples)
	return calc_entropy, n_samples
end
...
function get_entropy(y_predict, y)
	...
	# y[y_predict] get the values which is True
    entropy_true, n_true = entropy_of_one_division(y[y_predict])
	
	# right hand side entropy
	# y[.~y_predict] get the values which is False
...
function find_best_split(col_data, y)
    ...
    for value in Set(col_data)
		# y_predict is a list of True False
        y_predict = col_data .< value
        my_entropy = get_entropy(y_predict, y)
		
        #TODO
        #min entropy=?, cutoff=?
		min_entropy, cutoff = my_entropy < min_entropy ? (my_entropy, value) : (min_entropy, cutoff)
...
function _dtpredict(tree, row)
    cur_layer = tree
	# haskey check "cutoff" exist in cur_layer or not
    ...
    if !haskey(cur_layer, "cutoff")
		# Get "val" from cur_layer, if don't have return false
        return get(cur_layer, "val", false)
    end
end

# ╔═╡ 0d015f12-64b7-453f-bfb8-b9f35b866b18
# predict function from tree
...
function maxHypo(hist)
    ...
    #TODO
	max_hypo = findmax(hist)[2] # Get the key, not the value
    return max_hypo
end

# ╔═╡ 9a64e613-00be-456f-ac0c-72591cddd1f9
println(maxHypo(Dict([(2=>1), (3=>2), (1=>3)])))

# ╔═╡ 095e94ac-db7e-4029-9040-8131074dc96f
print(findmax(Dict([(2=>1), (3=>2), (1=>3)])))
...
function likelihood(_mean=nothing, _std=nothing ,data=nothing, hypo=nothing)
    ...
    res = res * prod(Gauss(std, mean, data))
    return res     
end
...
function gfit(X, y, _std=nothing, _mean=nothing, _hist=nothing)
    ...
    for hypo in Set(y)
        ...
        #TODO rows=
		# index = [i for i in 1:size(X,1) if y[i] == hypo]
		# rows = X[index, :]
		rows = X[y.==hypo, :]
		
        #histogram for each hypo
        #TODO probability=?
		#mean[hypo] = Statistics.mean.(eachcol(rows))
		#std[hypo] = Statistics.std.(eachcol(rows))
		#print(mean[hypo])
		#print(std[hypo])
		#print(rows)
		
		#probability = Gauss(reshape(std[hypo], (size(std[hypo], 1), 1)), reshape(mean[hypo], (size(mean[hypo], 1), 1)), rows)
...
begin
	...
	print("\nOur histogram after update X_test[10]: ", _gpredict(_hist, _mean, _std, X_test[10,:], true))
...
# ╔═╡ 59b01085-65a7-4044-b298-a97a94203aa4
function confusion_matrix(pred, y)
	num_classes = length(Set(y))
	num_samples = length(y)
	
	conf_matrix = zeros(num_classes, num_classes)
	for (pred_label, true_label) in zip(pred, y)
		conf_matrix[pred_label + 1, true_label + 1] += 1
	end
	return conf_matrix
end
...
begin
	# Row = predicted
	# Col = true
	y_pred = gpredict(_hist, _mean, _std, X_test)
	# Get confusion matrix of predictions
	conf_matrix = confusion_matrix(y_pred, y_test)
	print("Confusion matrix: \n", conf_matrix)
	num_classes = length(Set(y_test))

	# Calculate accuracy 
	_acc = sum([conf_matrix[i, i] / length(y_test) for i in 1:num_classes])
	# Calculate precision, recall and f1 of all classes using mean
	_p = mean([conf_matrix[i, i] / sum(conf_matrix[:, i]) for i in 1:num_classes])
	_r = mean([conf_matrix[i, i] / sum(conf_matrix[i, :]) for i in 1:num_classes])
	_f1 = mean((2 .* _p .* _r) ./ (_p .+ _r))
