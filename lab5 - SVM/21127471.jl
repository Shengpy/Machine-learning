...
student = (name = "Nguyễn Hoàng Anh Tuấn", id = "21127471")
...
function pla(pos_data::Matrix{Float64}, neg_data::Matrix{Float64}, 
	...
	# START YOUR CODE
	features = vcat(pos_data',neg_data') #dims=(size(pos_data,1),2))
	features = hcat(features, ones(size(features,1),1))
	y = [i <= size(pos_data,2) ? 1 : -1 for i = 1: size(features,1)]
	#num_input = size(features,2)
	#num_output = 1
	#d = Normal()
	#td = truncated(d, 0.0, sqrt(2.0 / (num_input + num_output)))
	theta = reshape([0,0,1],(3,1)) # Nen khoi tao 0,0
	for i = 1 : n_epochs
		values =  features * theta
		miss_predict_y = [ (1 - (values[i] == y[i] ? 1 : 0)) >= 0  ? 1 : -1 for i = 1: size(values,1)] 
		loss = 0
		for i in 1 : size(miss_predict_y, 1)
			if (miss_predict_y[i] == 1)
				theta = theta .+ η .* features[i,:] .* y[i]
			end
			loss += miss_predict_y[i]
		end
		if (loss == 0)
			return theta
		end
...
function eval_pla(θ, pos_data, neg_data)
	...
	f1 = 0
	for label in [-1,1]
		tp, fp, tn, fn = tpfptnfn_cal(y_test,y_pred,label)
...
Để $(\theta^{*})^\top\cdot\theta^{(k)}$ có thể đồng biến tuyến tính với nhau thì
$(\theta^{*})^\top\cdot\theta^{(k)} > (\theta^{*})^\top\cdot\theta^{(k-1)}$ 
hay nói cách khác 

$(\theta^{*})^\top\cdot\theta^{(k)}  - (\theta^{*})^\top\cdot\theta^{(k-1)} > 0$

rút gọn bất đẳng thức em có $(\theta^{*})^\top\cdot(\theta^{(k)}  - \theta^{(k-1)}) > 0$. Để tiện cho việc tính toán thì em gọi 1 biến mới delta = $\theta^{(k)}$  - $\theta^{(k-1)}$

Vậy em chỉ cần chứng minh $(\theta^{*})^\top \cdot delta > 0 (*)$ thì suy ngược lại em có thể kết luận được yêu cầu đặt ra.

Để $(*)$ thỏa thì 

$|(\theta^{*})^\top + delta|^2  >  |(\theta^{*})^\top - delta|^2$ 

Đặt $c = (\theta^{*})^\top$ . Nên em có :  

$c^{2} + delta^{2} + 2 * c \cdot delta > (c)^{2} + delta^{2} - 2 * c \cdot delta (1)$  
Rút gọn (1) với việc c là hằng số thì em có bất đẳng thức sau 

$c \cdot delta > - c \cdot delta (2)$ 

Với (2) luôn đúng thì em có thể kết luận được $(*)$ thỏa mãn do đó em có thể kết luận $(\theta^{*})^\top\cdot\theta^{(k)}$ đồng biến tuyến tính.
...
Từ đề bài em có được 

$||\theta^{(k)}||^2 = ||\theta^{k-1} + y_t\mathbf{x}_t||^2 (1)$ với 

$||\theta^{(k)}||^2 = ||(\theta^{k-1})||^2 + ||\mathbf{x}_t||^2 + 2 * (\theta^{k-1} \cdot y_t\mathbf{x}_t) (2)$

Giá trị $\theta^{(k)}$ thay đổi khi và chỉ khi $(\theta^{k-1} \cdot y_t\mathbf{x}_t)$ bé hơn 0 . Đồng thời $||(\theta^{k-1})||^2$ và $||\mathbf{x}_t||^2$ là độ dài của vector nên luôn lớn hơn 0, giả sử a = b + c + d với b, d > 0 và c < 0 thì việc bỏ c ra sẽ tạo thành bất đẳng thức a <= b + d, nên em có thể viết phương trình (2) thành bất đẳng thức sau :

$||\theta^{(k)}||^2 \leq ||(\theta^{k-1})||^2 + ||\mathbf{x}_t||^2 (3)$

Đồng thời theo đề thì $||\mathbf{x}_t|| \leq R (4)$

Kết hợp (3) và (4) thì em được tại lần thứ k cập nhập thì $||\theta^{(k)}||^2$ có thể tăng tối đa $||R||^2$. 
...
Đặt $\mathbf{x}$ thỏa $\mathbf{\theta}^\top\mathbf{x} + b = 1$ là $\mathbf{x}_+$. Do đó $\mathbf{x}_+ = \frac{(1 - b)}{\mathbf{\theta}^\top} (1)$

Đặt $\mathbf{x}$ thỏa $\mathbf{\theta}^\top\mathbf{x} + b = -1$ là $\mathbf{x}_-$.  Do đó $\mathbf{x}_+ = - \frac{(1 + b)}{\mathbf{\theta}^\top} (2)$

Với $\mathbf{x}_+$ và $\mathbf{x}_-$ là 2 điểm dữ liệu nằm ở mặt biên của 2 vùng lựa chọn, thì việc để có thể xác định khoảng cách giữa 2 vùng em có thể tính được thông qua tích vô hướng giữa vector đơn vị và hiệu giữa 2 điểm $\mathbf{x}_+$ và $\mathbf{x}_-$. Hay nói cách khác 

$\text{Distance} = (\mathbf{x}_+ - \mathbf{x}_-) \cdot \frac{\theta}{||\mathbf{\theta}||} (3)$

Thế (1) và (2) vào (3) em có được 

$\text{Distance} = (\frac{(1 - b)}{\mathbf{\theta}^\top} + \frac{(1 + b)}{\mathbf{\theta}^\top}) \cdot \frac{\theta}{||\mathbf{\theta}||} (4)$

Từ (4) rút gọn em được 

$\text{Distance} = \frac{2}{\mathbf{\theta}^\top} \cdot \frac{\theta}{||\mathbf{\theta}||} (5)$
$\text{Distance} =  \frac{2}{||\mathbf{\theta}||}(*)$

Vậy từ $(*)$ em có thể kết luận rằng khoảng cách giữa 2 điểm nằm trên mặt biên là $\frac{2}{||\mathbf{\theta}||}$ 
...
function hardmargin_svm(pos_data, neg_data, η=0.04, n_epochs=10000)
	...
	features = vcat(pos_data',neg_data')
	y = [i <= size(pos_data,2) ? 1 : -1 for i = 1: size(features,1)]
	w = [0.0,0.0]
	b = 0.0
	## Loss function
	previousSum = size(features,1);
	# println(previousSum)
	...
	for i = 1 : n_epochs
		tempW = w - η .* w
		labels = (features * tempW) 
		for i = 1 : size(labels,1)
			if(labels[i] * y[i] + b - 1 < 0)
				labels[i] = 1
			else
				labels[i] = 0
			end
		end
		if(sum(labels) > previousSum)
			println(sum(labels))
			println(previousSum)
			println("ok")
			return w,b
		else
			previousSum = sum(labels)
			w = tempW
			for i = 1 : size(labels,1)
				w = w + η .* (features[i,:] .* (labels[i] * y[i]))
			end
			b = b + η * dot(labels, y)
		end
	end
...
function softmargin_svm(pos_data, neg_data, n_epochs=10000, C=0.12, η=0.01)
	...
	b = 0.0
	slacks = zeros(size(y))
	previousSum = size(y,1)
	for i = 1 : n_epochs
		tempW = w - η .* w
		tempSLK = slacks .- η * C  
		labels = (features * tempW) 
		for i = 1 : size(labels,1)
			if(labels[i] * y[i] + b - 1 + tempSLK[i] < 0)
				labels[i] = 1
			else
				labels[i] = 0
			end
		...
		if(sum(labels) > previousSum)
			...
			b = b + η * dot(labels, y)
			for i = 1 : size(labels,1)
				if(slacks[i] < 0 || labels[i] == 1)
					slacks[i] = slacks[i] + η * 1.0 
				end
			end
		end
	end
...
# ╔═╡ a7a08ec4-c625-4018-b619-f34c1791ace4
function updatePair(x,y,alpha,a_r,a_s, kernal ,C=-1)
	a = -0.5 * kernal(x[a_r], x[a_r])
	b = 0
	c = sum(alpha) - alpha[a_r]
	leftover = y[a_r] * alpha[a_r]
	for i = 1 size(alpha,1)
		if(i != a_r)
			b += alpha[i] * y[a_r] * y[i] * kernal(x[a_r], x[i])
			if i != a_s
				leftover += y[i] * alpha[i]
			end
		end
	end
	leftover *= -1.0
	...
	for val in points
		...
		result = val * val * a + b * val + c
		maximum = max * max * a + b * max + c 
		if result > maximum 
			max = val
		end
	end
	...
	alpha[a_r] = max
	alpha[a_s] = (leftover - y[a_r] * alpha[a_r]) / y[a_s]
end
...
function dualsvm_smo_hard(pos_data, neg_data, n_epochs=100, λₜₒₗ=0.0001, errₜₒₗ=0.0001)
	...
	λ = zeros(length(D))#zeros(length(D))
	b = 0
	n = length(λ)
	pre = norm(λ)
	...
	for i = 1 : n_epochs
		...
		updatePair(X,Y,λ,a_r,a_s,K)
		out = true
		theta = zeros(size(X[1]))
		counter = 0
		for i = 1 : size(λ,1)
			if λ[i] > 0
				theta = theta .+ Y[i] * λ[i] .* X[i]
			end
		end
		for i = 1 : size(λ, 1)
			if λ[i] > 0
		...
		b = b / counter
		for i = 1 : size(λ,1)
			checker = Y[i] * (K(theta,X[i]) + b)
			if(λ[i] < 0)
				out = false
			end
			if(λ[i] == 0 && checker < 1- errₜₒₗ)
				out = false
			end
			if(λ[i] > 0 && (checker > 1 + errₜₒₗ || checker < 1 - errₜₒₗ))
				out = false
			end
		end
		if(out == true && abs(norm(λ) - pre) < λₜₒₗ)
			println("coverage")
			break
		end
		pre = norm(λ)
...
function dualsvm_smo_soft(pos_data, neg_data, n_epochs=100, C=1000, λₜₒₗ=0.0001, errₜₒₗ=0.0001)
	...
	for i = 1 : n_epochs
		...
		for i = 1 : size(λ,1)
			...
			if(λ[i] == C && checker > 1 + errₜₒₗ)
				out = false
			end
			if(λ[i] > 0 && λ[i] < C && (checker > 1 + errₜₒₗ || checker < 1 - errₜₒₗ))
				out = false
			end
		end
		if(out == true && abs(norm(λ) - pre) < λₜₒₗ)
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "6d59fe236abea56dedeab3a6a71dca2fffb73458"
