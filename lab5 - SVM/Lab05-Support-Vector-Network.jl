...
student = (name = "Lê Nhựt Nam", id = "20xx2023")
...
function pla(pos_data::Matrix{Float64}, neg_data::Matrix{Float64}, 
    ...
    θ = zeros(Float64, n_features + 1)

    # Add bias term to the data
    pos_data_bias = hcat(pos_data',ones(Float64,size(pos_data', 1)))
    neg_data_bias = hcat(neg_data',ones(Float64,size(neg_data', 1)))

    # Combine positive and negative data
    all_data = vcat(pos_data_bias, neg_data_bias)'
	
    # Combine labels for positive and negative data
    labels = vcat(ones(size(pos_data, 2)), -ones(size(neg_data, 2)))'
	
    # PLA training loop
    for epoch in 1:n_epochs
        # Shuffle the data and labels
        shuffle_indices = shuffle(1:length(labels))
        shuffled_data = all_data[:,shuffle_indices]
        shuffled_labels = labels[shuffle_indices]

        # PLA update rule
        for i in 1:size(shuffled_data, 2)
            x = shuffled_data[:,i]
            y = shuffled_labels[i]
            if sign(θ' * x) != y
                θ += η * y * x
...
https://stats.stackexchange.com/questions/425169/distance-between-2-hyperplanes-in-svm-formulation
...
function hardmargin_svm(pos_data, neg_data, η=0.04, n_epochs=10000)
    ...
    neg_data=neg_data'
	# Get the number of features
    n_features = size(pos_data, 2)

	## Create variables for the separating hyperplane w'*x = b.
    w = zeros(Float64, n_features+1)
    b = 0.0
	# println(size(w))
    ...
    # Combine labels for positive and negative data
    labels = vcat(ones(size(pos_data, 1)), -ones(size(neg_data, 1)))'
    # println(size(all_data))
    ...
    function Loss(w)
		...
		# println(size(x*w))
		# println(size((x * w) .- y'))
		# println(size((x'*(x*w .- y')) * 1/N))
		# println(size(x' * ((x .* w) .- y)))
		# return x' * ((x .* w) .- y)
		return (x'*(x*w .- y')) * 1/N
	end
   ...
   #              # w -= y * x
   #              # b -= y
			# 	w = w .- η * Loss(size(all_data,2),w,x,y)
   #          end
   #      end
	##converged
		print(size((w' * all_data) .+ b))
	    if all(y*((w * all_data) .+ b) .>= 1.0)
            break
        end
	## Update weight
        # w -= η * w
