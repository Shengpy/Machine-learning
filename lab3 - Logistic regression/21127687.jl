...
# ╔═╡ e3fc0f90-c66f-11ed-047d-21c665f6f6c7
begin
	using  Distributions, Plots, Images, LinearAlgebra, Random
end

# ╔═╡ 64408691-760f-472b-a91f-4ecee02e7bd1
md"""
...
# ╔═╡ 0168bb87-adf3-4752-be61-88d77addb685
md"""
...
# ╔═╡ 93f55726-257b-47fa-ac72-310ebfebd035
md"""
## 1. Feature Extraction
"""

# ╔═╡ ec419c4a-6d41-4b5d-bedc-dac432e208ce
md"""
### Import Library
"""

# ╔═╡ efd3cf67-9670-4e80-a0da-99aa888824cf
Random.seed!(2024)

# ╔═╡ 4baa581b-489e-4aca-a2b4-ad1cd26c232d
md"""
### Load data
"""

# ╔═╡ 085459dc-4dc2-4999-915f-672812e92649
...
# ╔═╡ bdb4ac6c-af66-4fdd-a907-b11005d4332c
...
# ╔═╡ c3d7e240-a254-4967-8e99-bc405fb093b3
begin
...
# ╔═╡ 257d4f03-efd6-4a82-95e9-a396d112f104
begin
	# Init arrays
...
# ╔═╡ bcb942f5-8e4e-4a98-84fc-3c025b36dc35
begin
	# Init arrays
	...
	# Close io streams
	close(io_images_test)
	close(io_labels_test)

	# Transpose features
	test_x = test_x'
end

# ╔═╡ a987a69c-89e4-44be-bf1e-0ca9f8463e5f
size(train_x), size(train_y), size(test_x), size(test_y)

# ╔═╡ 434b1b2d-3473-4c9e-9284-162ac29fb8bd
md"""
### Extract Features
"""

# ╔═╡ 2f04dc8a-a483-4025-a293-faa5f83b9256
md"""
...
# ╔═╡ 92315aac-12e7-45cf-85b7-78b50137f7f9
...
function compute_average_intensity(x, y)
	...
	for row in eachrow(x)
		...
		index = y[i] + 1
		count_[index] += 1
		mean_[index] = (mean_[index] + temp) / count_[index]
	end
    return mean_
end

# ╔═╡ d3534ddb-8ffa-406b-8b3d-2e944400df43
l_mean = compute_average_intensity(train_x, train_y)

# ╔═╡ ec37b667-17f1-4da2-84ed-a38b86de692a
md"""
Plot the average intensity using matplotlib
"""

# ╔═╡ 805ec630-a94b-4ccd-a6a9-7659063c6fb7
bar(0:9, l_mean, legend=false)

# ╔═╡ 3696c7a0-6d49-4669-a551-154006e606bc
begin
	#TODO compute average intensity for each data sample
	intensity = [mean(i) for i in eachrow(train_x)] 
	#append!(intensity, [mean(i) for i in eachrow(test_x)])
	size(intensity)
	print(intensity)
end

# ╔═╡ 99ed1c6a-02a1-49f7-bfac-0aa652620677
md"""
...
# ╔═╡ 28cb618e-e3f1-4b75-81d6-00ddc903aaf7
...
# ╔═╡ 3a85e9d6-2186-4df7-94e0-2246d9880189
begin
	symmetry = compute_symmetry(train_x)
	#append!(symmetry, compute_symmetry(test_x))
	size(symmetry)
end

# ╔═╡ ae707bca-4feb-418c-9a8e-069e21201738
md"""
Visualize 10 samples in order to illustrate symmetry
"""

# ╔═╡ d26608b9-cc61-4f1d-8f68-a7b0a749080e
begin
	num_img = 10
...
# ╔═╡ b7da254b-fde3-42a2-becb-5ec925eb2b34
begin
...
# ╔═╡ c4804caa-cfe2-4162-abc3-fdfd11cfeab8
md"""
Our new data will have 70000 samples and 2 features: intensity, symmetry.
"""

# ╔═╡ 23c8dad6-ab99-4d8a-a74d-a9adf6eaebac
begin
...
# ╔═╡ 26e2a6f5-fb8c-4564-90f8-14821bee9bd2
md"""
## 2. Training 
"""

# ╔═╡ c9fe6f43-0198-479b-a213-e2a236a59961
md"""
...
# ╔═╡ 7f02f99a-6f46-45b1-b7c3-475c8b492fdb
md"""
### Normalize data
First normalize data using Z-score normalization
- **TODO: Study about Z-score normalization**
	Z-score normalization, otherwise known as standardization, refers to the normalization method using probability and statistics. Assuming the datapoint that needs normalizing is x. The formula is as follow
$$\mathbf{x} - \mu / \delta$$
	mu represents the mean of the dataset we're working with, and delta represents the standard deviation of the data. This process assigns the newly calculated z value to our x datapoint, bounding it in the range [0, 1].
- **TODO: Why should we normalize data?**
	Although logistic regression does not assume normality in its data, it still has quite the effect on other models if the data is skewed. Working with skewed data could result in inaccurate model training and evaluation. Another reason is that normalization puts every weighted feature on a similar scale. If a feature's mean value is 100k+ whereas another's mean is a measly 5, the model could completely disregard the latter feature due to its small weight compared to the previous. Some normalization methods will transform these features' value range to a more appropriate scale, thus allowing the model to train and produce better results. 
"""

# ╔═╡ 12f92c2e-e9d1-40f6-aad8-175fb5481b54
function normalize(x, mean_=nothing, std_=nothing)
   if mean_ == nothing && std_ == nothing
	   ...
	   #mean_ = mean(x)
	   mean_ = [mean(i) for i in eachcol(x)]
	   #std_ = std(x)
	   std_ = [std(i) for i in eachcol(x)]
	   normalized_train_x = copy(x)
	   for i in eachrow(normalized_train_x)
		   i .= (i .- mean_) ./ std_
	   end
	   return normalized_train_x, mean_, std_
    end
...
# ╔═╡ c3046f33-1e9b-4830-946f-3193c3515149
begin
...
# ╔═╡ 8a596c7e-f04d-49ca-9586-29a039a4a3aa
md"""
### Construct  data
"""

# ╔═╡ 2f79e2e8-2295-42fd-971f-b6b710441468
begin
	print(train_y, "\n")
	train_y_new = reshape(deepcopy(train_y), (size(train_y)[1], 1))
	#print(size(train_y), size(train_y_new))	
...
# ╔═╡ 701e483f-4b55-4ebc-9969-fb4e09a1a7f9
begin
...
# ╔═╡ c0a1f518-88c7-4ac0-9486-c0825b069822
md"""
### Sigmoid function and derivative of the sigmoid function
"""

# ╔═╡ ed17372b-0c96-46e6-80a4-3281fcff4440
...
# ╔═╡ d74c94b7-953a-4011-881d-f3634bd2df8a
#print(sigmoid_activation([3, 4, 5]))

# ╔═╡ 458316fb-ebdb-4f17-a11b-b00bc63bf4f6
...
# ╔═╡ bf2a44b0-4bde-4a6f-be13-3a57b88a2885
md"""
### Compute output
"""

# ╔═╡ e65a06ea-b346-4774-871f-29423b1cb7e1
...
# ╔═╡ 9775e591-54d4-469c-a6d9-f0ea69b7c6b9
...
# ╔═╡ a8555edc-546a-4226-ad67-830e0d814e02
md"""
### Compute gradient
"""

# ╔═╡ 40eca327-dc63-40c1-b061-a922bea80e42
md"""
...
# ╔═╡ 72ccfb7d-fdbe-4342-90e5-00f02e7ecbf8
function compute_gradient(error, train_x)
    ...
    """
    return train_x' * error
end

# ╔═╡ d6fbe31f-0a62-47dc-825d-5785392e7c55
function train(W, train_x, train_y, learning_rate, num_epochs)
    ...
    for epoch in 1:num_epochs
		...
		#print(y_hat, "\n")
        error = y_hat - train_y
		cross_entropy = mean(-1. .* train_y .* log.(y_hat) .- (1. .- train_y) .* log.(1. .- y_hat))
		#print(cross_entropy)
        append!(losses, cross_entropy)
...
# ╔═╡ 1590dd5b-258f-4488-a842-e5df1def6bec
md"""
### Train our model
"""

# ╔═╡ c4157864-3286-4185-91ad-1bf3de10df6f
begin
	W = rand(Normal(), (size(add_one_train_x)[2], 1))

	#num_epochs=200 # - Converges at epoch ~150
	num_epochs = 200
	learning_rate=0.000001
	for i in 1:size(train_y_new, 1)
		print(train_y_new[i], " - ")
		print(train_y[i], "\n")
	end
...
# ╔═╡ 864aebd4-984a-46e2-999a-956ced68292e
plot(1:num_epochs, losses, legend=false)

# ╔═╡ a8a5322a-3d86-44b7-8a1f-354bf1ffaed0
md"""
...
# ╔═╡ cf5e486e-0e02-466c-b240-b363d002506c
md"""
### Evaluate model on training set
"""

# ╔═╡ c79d1554-870f-4cc5-88ce-ccac2279af88
...
# ╔═╡ 54e2a0b9-afcf-448e-b5ab-bbb9f82522e6
begin
	preds_train = predict(W, add_one_train_x)
	#print(preds_train[6][1], "\n")
	...
	for i ∈ 1:10
		# Calculate true positives, false positives, false negatives, and true negatives
	    true_positives, false_positives, true_negatives, false_negatives = tpfptnfn_cal(train_y_n, preds_train)
...
# ╔═╡ dd9da959-8a0a-43f8-a916-6011c85a6741
md"""
...
begin
	...
	print(size(add_one_test_x))
end

# ╔═╡ e6897bd2-1d5d-45a2-8b30-67d1951fa12e
md"""
After doing all these stuffs, you now can predict and evaluate your model
"""

# ╔═╡ d2d54f7b-b225-4a98-8175-39fcd9df905e
begin
...
# ╔═╡ b052e52d-b95d-4dde-9fc2-74a277916801
md"""
**TODO: Comment on the result**

	The initial learning rate 0.01 would perform poorly as the gradient would be too large, and after only 2,3 epochs, the cross-entropy value would return NaN due to the expression $log(y)$ with $y \approx 0$. Hence I adjusted the learning rate to be lower. With this, the training of the model will converge before 500 epochs and reach the average accuracy score of 92.6% on the test set.
"""
